/* ========================================================================
   NiNi — stage.js
   Layout 3 cột + Router hash + 6 mục (Video, Game, Shop, Chat, Thông báo, Hồ sơ)
   - Cột trái: sidebar icon (cố định)
...
/* --------------------------- [0] Demo data (mock) ----------------------- */
const DEMO_VIDEOS = [
  { id: "v1", title: "AI có mơ chăng?", url: "https://www.youtube.com/embed/StFihkKIywo?si=7QSX371UAAXk-seX" },
  { id: "v2", title: "Tua video 2",     url: "https://www.youtube.com/embed/ohq5S9YRHA0" },
];
...
/* --------------------------- [1] Auth helpers --------------------------- */
/** Lấy current user “an toàn”, ưu tiên cache NiNiAuth.user (được bridge set sớm), fallback sang wrapper fb */
function getCurrentUser() {
  const u1 = (window.NiNi && window.NiNi.user) || (window.NINI && window.NINI.user);
  if (u1) return u1;
  const fb = (window.NiNi && window.NiNi.fb) || (window.NINI && window.NINI.fb) || (window.N && window.N.fb);
  if (!fb) return null;
  try {
...
function renderHomeRight() {
  ...
  `;
  if (typeof initReaderBehavior === "function") initReaderBehavior();
}
...
/* Re-render Profile khi auth đổi trạng thái (đảm bảo vào /#/Profile thấy ngay) */
...
/* Nếu wrapper có onAuthStateChanged, hook thêm để chắc chắn không miss tick đầu */
setTimeout(() => {
...
window.NiNiStory = { book: null, pages: [], idx: 0, lang: "vi" };

async function loadLibraryTo(selector) {
  const host = "/public/content/storybook";
  let manifest;
  try { manifest = await fetchJSON(`${host}/library-manifest.json`); }
  catch {  manifest = await fetchJSON(`${host}/library-menifest.json`); }
